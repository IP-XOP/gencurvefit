...
Static Function GEN_panelDofitbutton(ctrlName) : ButtonControl
	...
	String cmd,cursors,mask,errors,fitfunction,coefs,limits, y_wavename,x_Wavename,e_wavename,holdstring=""
	string mask_wavename,options
	variable usewavescaling,useerrors,usemask,usecursors
	...
	coefs = "root:motofit:GEN_optimise:GEN_tempparwave"
	...
	if(cmpstr(S_Value,"_none_")==0)
		...
		y_wavename = possiblyquotename(S_Value)
	endif
	...
	elseif(cmpstr(S_Value,"choose again") == 0)
		...
		x_wavename = possiblyquotename(S_Value)
	endif
	...
	elseif(cmpstr(S_Value,"_none_")==0)
		...
		useerrors=1
		e_wavename = possiblyquotename(S_Value)
	endif
	...
	controlinfo/W=geneticoptimisation usemaskwave
	mask_wavename = possiblyquotename(S_Value)
	usemask=cmpstr(S_Value,"_none_")
	...
	try	//the user might abort halfway through						
		...
		if(itemsinlist(Operationlist("GENcurvefit",";","external")))
			...
			cmd += ",root:motofit:GEN_optimise:GENcurvefitlimits"
		else
			cmd ="GEN_curvefit("
			cmd += "\""+fitfunction+"\""
			cmd += ","+coefs
		...
		if(V_Value)
			...
			if(waveexists($fitxname))
				Appendtograph/w=$topgraph $fitname vs $fitxname
			else
				Appendtograph/w=$topgraph $fitname
			endif
		endif 
		
		//update the listbox
		//this wave contains the best fit at the end
		Wave GEN_parwave=root:motofit:GEN_optimise:GEN_tempparwave
...
Static Function GEN_checkinitiallimits(GEN_limits,GEN_b)
	...
	Wave GEN_parnumber = root:motofit:GEN_optimise:GEN_parnumber 
...
Function GEN_setlimitsforGENcurvefit(coefs,holdstring,GEN_calldatafolder)
	...
	String/g root:motofit:GEN_optimise:callfolder = GEN_calldatafolder
	do
		try    //the user may want to abort the fit at this stage and we need to return to the right DF
			GEN_setlimitwaveGENcurvefit(coefs,holdstring,numvarparam) 
			Wave/z GENcurvefitdummylimits = root:motofit:GEN_optimise:GENcurvefitdummylimits
			Wave/z GENcurvefitdummycoefs = root:motofit:GEN_optimise:GENcurvefitdummycoefs
	...
	make/n=(numpnts(coefs),2)/o root:motofit:GEN_optimise:GENcurvefitlimits
	Wave GENcurvefitlimits = root:motofit:GEN_optimise:GENcurvefitlimits 
	jj=0
	for(ii=0;ii<numpnts(coefs);ii+=1)
		if(cmpstr(holdstring[ii],"0")==0)
...
Function GEN_setlimitwaveGENcurvefit(coefs,holdstring,numvarparam) 
	...
	Wave/z GENcurvefitlimits = root:motofit:GEN_optimise:GENcurvefitlimits
	Wave/z GENcurvefitdummylimits = root:motofit:GEN_optimise:GENcurvefitdummylimits
	Wave/z GENcurvefitdummycoefs = root:motofit:GEN_optimise:GENcurvefitdummycoefs
	...
	make/o/d/n=(numvarparam,1) root:motofit:GEN_optimise:GENcurvefitdummycoefs, root:motofit:GEN_optimise:Gen_parnumber
	make/o/d/n=(numvarparam,2) root:motofit:GEN_optimise:GENcurvefitdummylimits 
			      
	Wave/z GENcurvefitdummylimits= root:motofit:GEN_optimise:GENcurvefitdummylimits 
	Wave/z GENcurvefitdummycoefs= root:motofit:GEN_optimise:GENcurvefitdummycoefs
	Wave/z Gen_parnumber= root:motofit:GEN_optimise:Gen_parnumber
	jj=0
	for(ii=0;ii<numpnts(coefs);ii+=1)
		if(cmpstr(holdstring[ii],"0")==0)
			Gen_parnumber[jj] = ii 
